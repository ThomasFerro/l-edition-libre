...
func ExtractUserID(token string) (contexts.UserID, error) {
	request, err := http.NewRequest("GET", Auth0UserInfoURL, nil)
	if err != nil {
		return contexts.UserID(""), fmt.Errorf("unable to create user info request: %v", err)
	}
	request.Header.Add("Authorization", fmt.Sprintf("Bearer %v", token))
	response, err := http.DefaultClient.Do(request)
	if err != nil {
		return contexts.UserID(""), fmt.Errorf("unable to get user info: %v", err)
	}
	defer response.Body.Close()

	err = handleHttpError(response)
	if err != nil {
		return contexts.UserID(""), fmt.Errorf("user info error: %v", err)
	}

	body, err := io.ReadAll(response.Body)
	if err != nil {
		return contexts.UserID(""), fmt.Errorf("user info body reading error: %v (body: %v)", err, body)
	}
	var payload Payload
	err = json.Unmarshal(body, &payload)
	if err != nil {
		return contexts.UserID(""), fmt.Errorf("user info body unmarshal error: %v (body: %v)", err, string(body))
	}
	return contexts.UserID(payload.Sub), nil
}
